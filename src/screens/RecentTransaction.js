...
const RecentTransaction = ({ navigation, transaction }) => {
  const [sortedTransactions, setSortedTransactions] = useState([]);

  useEffect(() => {
    if (transaction && transaction.length > 0) {
      const sorted = [...transaction].sort((a, b) => {
        // Assuming 'date' field is a valid date string, you may need to adjust sorting logic if 'date' is not in a sortable format
    ...
    <View style={styles.container}>
      ...
      {sortedTransactions && sortedTransactions.length > 0 ? (
        ...
        <View style={styles.iconContainer}>
          <Ionicons style={styles.icon} name="notifications-outline" size={24} color="white" />
